...
namespace frb
{
  ...
  * @note       
  */
  bool Lift::is_sensor_on(DISignal signal)
  {
  ...
  /**
  * @brief      lift의 sensor data가 error 상태인지 확인
  * @return     bool : lift의 fall, raise sensor 두개 모두가 on 상태인지
  * @note       추후 다른 sensor 추가시 수정 필요
  */
  bool Lift::is_sensor_error()
  {
  ...
  */
  void Lift::execute_fall_action()
  {
    ...
    {
      log_msg(LogInfo, 0, "Lift initialize fall action start -- ");
      _motor->control_move(0, _action_rpm/3*2, CHECK_NONE, TIMEOUT_NONE);
    }
    else
    {
      log_msg(LogInfo, 0, "Lift fall action start -- ");
  ...
  */
  void Lift::execute_raise_action()
  {
    if(!is_controlable())
    {
    ...
    {
      log_msg(LogInfo, 0, "Lift initialize raise action start -- ");
  ...
  *   
  */
  void Lift::execute_middle_action()
  {
    if(_state != frb::UnitState::Normal)  
    {
      log_msg(LogError, 0, "Error : lift is not initialized");
      return;
    }

    if(!is_controlable())                         
    {
      log_msg(LogInfo, 0, "Lift is not controlable");
      return;
    }
    ...
    log_msg(LogInfo, 0, "Lift middle action start -- ");
